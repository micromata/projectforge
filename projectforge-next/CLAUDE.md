...
- **Explanatory hover text goes through `HintTooltip`** (`components/shared/hint-tooltip.tsx`) — never the browser's native `title="…"` (grey OS box, no wrapping, not announced). Wrap the explained element as its single child; it renders nothing when there is no text, so an optional backend tooltip may be passed straight through. Pass `openOnTap` for a passive trigger so the explanation is reachable on touch; leave it off where the trigger is itself actionable (a button, a link, a clickable cell).
