...
export function OrderEditPageClient() {
  const raw = useRouteParams<{ id: string }>("/order/[id]")?.id;
  ...
  // "new" adds an order — the same form, just without an id to load.
  const isNew = raw === "new";
  const id = Number(raw);
  ...
  return (
    <PageShell>
      <EntityEditPage page={ORDER_PAGE} id={isNew ? null : id} />
    </PageShell>
  );
}
