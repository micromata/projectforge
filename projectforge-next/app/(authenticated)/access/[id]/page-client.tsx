...
export function AccessEditPageClient() {
  const raw = useRouteParams<{ id: string }>("/access/[id]")?.id;
  ...
  // "new" adds an access entry — the same form, just without an id to load.
  const isNew = raw === "new";
  const id = Number(raw);
  ...
  return (
    <PageShell>
      <EntityEditPage page={ACCESS_PAGE} id={isNew ? null : id} />
    </PageShell>
  );
}
