...
export function GroupEditPageClient() {
  const raw = useRouteParams<{ id: string }>("/group/[id]")?.id;
  ...
  // "new" adds a group — the same form, just without an id to load.
  const isNew = raw === "new";
  const id = Number(raw);
  ...
  return (
    <PageShell>
      <EntityEditPage page={GROUP_PAGE} id={isNew ? null : id} />
    </PageShell>
  );
}
