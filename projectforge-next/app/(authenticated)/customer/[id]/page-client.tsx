...
export function CustomerEditPageClient() {
  const raw = useRouteParams<{ id: string }>("/customer/[id]")?.id;
  ...
  // "new" adds a customer — the same form, just without an id to load.
  const isNew = raw === "new";
  const id = Number(raw);
  ...
  return (
    <PageShell>
      <EntityEditPage page={CUSTOMER_PAGE} id={isNew ? null : id} />
    </PageShell>
  );
}
