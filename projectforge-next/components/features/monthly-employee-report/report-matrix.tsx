...
import type { MonthlyReport, MonthlyReportRow } from "./types";

/**
 * The report matrix: rows are cost units / tasks, columns the calendar weeks, plus a monthly sum and — when
 * enabled — the time saved by AI. A data row drills down into the time sheet list filtered by the reported
 * user, the exact cost unit (or task) and the month range (see lib/timesheet-links.ts).
 */
export function ReportMatrix({ report }: { report: MonthlyReport }) {
  const t = useTranslations();
  const navigate = useNavigateMenuUrl();

  function drillDown(row: MonthlyReportRow) {
    navigate(
