...
The gateway instance's `projectforge.properties` requires only the gateway mode and database connection.
...
For testing with HSQLDB (no external database needed):
