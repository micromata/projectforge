...
ProjectForge supports authentication via an external OAuth2/OIDC provider.
Users log in through the identity provider (e.g. Authentik, Keycloak) and are redirected back to ProjectForge.
The user must already exist in ProjectForge (matched by `idpExternalId` or username).

==== Configuration

Add the following OAuth2/OIDC client configuration to `application.properties`:
...
Replace `authentik` with your provider's registration name (e.g. `keycloak`) and configure the issuer URI accordingly.

==== User mapping

On successful OAuth2 login, ProjectForge resolves the local user account as follows:

. Match the OIDC `sub` claim against the `idpExternalId` field of ProjectForge users
. If no match is found, fall back to matching the `preferred_username` claim against the ProjectForge username
. If still no match: login is rejected

Deactivated users are always rejected regardless of a valid OAuth2 session.

==== External Gateway mode

ProjectForge can run as a reduced, public-facing instance that authenticates *exclusively* via OAuth2/OIDC.
In this mode only the following services are exposed:

* CardDAV (address book sync)
* CalDAV / ICS subscriptions (calendar export)
* DataTransfer (file exchange areas)

Activate with the Spring profile `external-gateway`:
...
The gateway instance's `projectforge.properties` requires the gateway mode and the database connection.
All other gateway settings (CardDAV, menu visibility) are provided by the `external-gateway` profile automatically.

The OAuth2/OIDC configuration is only needed for the DataTransfer UI: CardDAV and CalDAV/ICS authenticate
via tokens and work without it. If the OAuth2 client is not configured, the gateway starts without a login option.

NOTE: ProjectForge adds the home `projectforge.properties` as `--spring.config.additional-location`,
which Spring loads with the highest priority. It therefore overrides the `external-gateway` profile,
so keep this file limited to the settings below rather than repeating profile defaults.

[source,properties]
----
# Public URL of the gateway, including the port if published directly.
# This is the {baseUrl} of the OAuth2 redirect URI below.
projectforge.domain=https://gateway.example.com

# Gateway mode
projectforge.gateway.enabled=true
projectforge.gateway.sync.secret=<shared-secret>

# PostgreSQL (adjust host/credentials to your setup)
...
projectforge.security.authenticationTokenEncryptionKey=<your-key>

# OAuth2/OIDC (only required for the DataTransfer UI)
spring.security.oauth2.client.registration.authentik.client-id=${AUTHENTIK_CLIENT_ID}
spring.security.oauth2.client.registration.authentik.client-secret=${AUTHENTIK_CLIENT_SECRET}
spring.security.oauth2.client.registration.authentik.scope=openid,profile,email
spring.security.oauth2.client.registration.authentik.authorization-grant-type=authorization_code
...
For testing with HSQLDB (no external database needed), replace the PostgreSQL block:
...
The internal ProjectForge instance syncs user data to the gateway via the shared secret.

===== Main instance push configuration

The main (internal) ProjectForge instance must be configured to push user, group, address and calendar data to the gateway.
Add the following to the main instance's `projectforge.properties`:

[source,properties]
----
projectforge.gateway.push.enabled=true
projectforge.gateway.push.url=https://gateway.example.com/api/gateway/sync
projectforge.gateway.push.secret=<shared-secret>
...
|`push.enabled` |Enable periodic sync push to the gateway instance
|`push.url` |Base URL of the gateway's sync API endpoint
|`push.secret` |Shared secret (must match `projectforge.gateway.sync.secret` on the gateway)
|`push.syncIntervalMs` |Interval between sync pushes in milliseconds (default: 900000 = 15 minutes)
|===

The push service sends users, groups, address books, and cached ICS calendar data to the gateway.
All requests are authenticated via the `X-Gateway-Secret` HTTP header.

=== Identity Provider Sync (IdP)

ProjectForge can act as a master for user and group data, provisioning them into an external identity provider.
This is independent of the OAuth2 login described above — it handles user/group lifecycle management.
Currently supported providers are *Keycloak* and *Authentik*.

==== Configuration

[source,properties]
----
# Provider: "keycloak" (default) or "authentik"
projectforge.idp.provider=keycloak
projectforge.login.handlerClass=IdpMasterLoginHandler

# Optional: sync passwords to IdP on successful login
...
. Create a realm (e.g. `acme.com`)
. Enable locales globally: `de`, `en`
. Create a client `projectforge` (confidential, service account enabled)
...
projectforge.keycloak.pageSize=100
----

===== User attributes

Create the following custom user attributes in the Keycloak realm:
...
Set the username validator minimum length to *2* (Keycloak default is 3; reduced for short usernames like `jo`).

IMPORTANT: Users must *not* be allowed to edit their own attributes (name, first name, etc.) in Keycloak.

==== Authentik setup

. Create a new role (e.g. `projectforge-sync`) with permissions: Group (add, change, delete, view), User (add, change, delete, view, add/remove user to group)
. Create a sync user in Admin with this role
...
projectforge.authentik.pageSize=100
----

Authentik stores custom attributes as a JSON object on users/groups — no pre-configuration of attribute schemas is needed.

==== Attribute mappings

Configure which ProjectForge fields are synced to IdP user/group attributes:
...
|`LoginDefaultHandler` |No external IdP (default)
|`IdpMasterLoginHandler` |ProjectForge is master; users/groups are pushed PF -> IdP
|`IdpLoginHandler` |IdP is master; users/groups are pulled IdP -> PF
|===
