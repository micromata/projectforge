    ...
    companion object {
        ...
        fun createEditLayout(book: BookDO?): UILayout {
            ...
            val layout = UILayout(titleKey)
                    .add(UIGroup().add(UIInput("title", required = true, focus = true, label = "@")))
                    .add(UIGroup().add(UIInput("authors", label = "@")))
                    .add(UIRow()
                            .add(UICol(6)
                                    .add(UIGroup().add(UISelect("type", i18nEnum = BookType::class.java, required = true, label = "@")))
                                    .add(UIGroup().add(UIInput("yearOfPublishing", label = "@")))
                                    .add(UIGroup().add( UISelect("status", i18nEnum = BookStatus::class.java, required = true, label = "@")))
                                    .add(UIGroup().add(UIInput("signature", label = "@"))))
                            .add(UICol(6)
                                    .add(UIGroup().add(UIInput("isbn", label = "@")))
                                    .add(UIGroup().add(UIInput("keywords", label = "@")))
                                    .add(UIGroup().add(UIInput("publisher", label = "@")))
                                    .add(UIGroup().add(UIInput("editor", label = "@")))))
                    .add(UIGroup()
