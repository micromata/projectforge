...
public class BookListForm extends AbstractListForm<BookListFilter, BookListPage> {
  ...
  protected TextField<?> createSearchTextField() {
    @SuppressWarnings({"unchecked", "rawtypes"}) final PFAutoCompleteTextField<BookDO> searchField = new PFAutoCompleteTextField<BookDO>(InputPanel.WICKET_ID,
            new Model() {
              @Override
              public Serializable getObject() {
                // Pseudo object for storing search string (title field is used for this foreign purpose).
                BookDO book = new BookDO();
                book.setTitle(getSearchFilter().getSearchString());
                return book;
              }
