...
public class InputCellItemListenerPropertyColumn<T> extends PropertyColumn<T, String>
{
   ...
   */
  public InputCellItemListenerPropertyColumn(final String displayModelString, final String sortProperty,
  ...
  public InputCellItemListenerPropertyColumn(final IModel<String> displayModel, final String sortProperty,
      final String propertyExpression,
      final CellItemListener<T> cellItemListener)
  {
  ...
  public InputCellItemListenerPropertyColumn(final Class<?> clazz, final String sortProperty,
      final String propertyExpression,
      final CellItemListener<T> cellItemListener)
  {
  ...
  public void populateItem(final Item<ICellPopulator<T>> item, final String componentId, final IModel<T> rowModel)
  {
    ...
    if (object == null) {
      item.add(new InputPanel(componentId, new TextField(InputPanel.WICKET_ID, propertyModel)));
    } else if (object instanceof I18nEnum) {
      item.add(new InputPanel(componentId, new TextField(InputPanel.WICKET_ID,
          new Model<String>(ThreadLocalUserContext.getLocalizedString(((I18nEnum) object).getI18nKey())))));
    } else if (object instanceof java.sql.Date) {
      item.add(new InputPanel(componentId, new TextField(InputPanel.WICKET_ID,
          new Model<String>(DateTimeFormatter.instance().getFormattedDate(object)))));
    } else if (object instanceof Date) {
      item.add(new InputPanel(componentId, new TextField(InputPanel.WICKET_ID,
          new Model<String>(DateTimeFormatter.instance().getFormattedDateTime((Date) object)))));
    } else {
      item.add(new InputPanel(componentId, new TextField(InputPanel.WICKET_ID, propertyModel)));
    }
