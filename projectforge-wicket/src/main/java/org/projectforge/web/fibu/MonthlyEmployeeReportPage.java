...
public class MonthlyEmployeeReportPage extends AbstractStandardFormPage implements ISelectCallerPage {
    ...
    public MonthlyEmployeeReportPage(final PageParameters parameters) {
        ...
        gridBuilder.newSplitPanel(GridSize.COL25);
        {
            final FieldsetPanel fs = new FieldsetPanel(gridBuilder.getPanel(), getString("vacation.annualleave")) {
                ...
                @Override
                public boolean isVisible() {
                    return report != null && StringUtils.isNotBlank(report.getFormattedVacationCount()) && WicketSupport.get(VacationService.class).hasAccessToVacationService(
        ...
        gridBuilder.newSplitPanel(GridSize.COL25);
        {
            final FieldsetPanel fs = new FieldsetPanel(gridBuilder.getPanel(), getString("vacation.plandannualleave")) {
                ...
                @Override
                public boolean isVisible() {
                    return report != null && StringUtils.isNotBlank(report.getFormattedVacationPlandCount()) && WicketSupport.get(VacationService.class).hasAccessToVacationService(
                            ThreadLocalUserContext.getLoggedInUser(), false);
                }
            }.suppressLabelForWarning();
            fs.add(new DivTextPanel(fs.newChildId(), new Model<String>() {
    ...
    protected void exportAsPdf() {
        ...
        final PFUserDO employee = WicketSupport.getUserDao().find(form.filter.getUserId());
        ...
        data.put("systemDate", WicketSupport.get(DateTimeFormatter.class).getFormattedDateTime(new Date()));
