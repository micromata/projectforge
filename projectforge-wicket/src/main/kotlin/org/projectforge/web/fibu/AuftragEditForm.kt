...
open class AuftragEditForm(parentPage: AuftragEditPage?, data: AuftragDO) :
    ...
    override fun init() {
        ...
        gridBuilder.newFieldset(getString("fibu.auftrag.nummer")).let { fs ->
            // Number
        ...
        gridBuilder.newFieldset(getString("fibu.auftrag.nettoSumme")).suppressLabelForWarning().let { fs ->
            // Net sum
        ...
        gridBuilder.newFieldset(getString("fibu.auftrag.title")).let { fs ->
            // Title
        ...
        gridBuilder.newSplitPanel(GridSize.COL50)
        gridBuilder.newFieldset(getString("fibu.common.customer.reference")).let { fs ->
            // reference
            fs.add(MaxLengthTextField(InputPanel.WICKET_ID, PropertyModel(data, "referenz")))
        }
        gridBuilder.newSplitPanel(GridSize.COL50)
        gridBuilder.newFieldset(getString("status")).let { fs ->
        ...
        gridBuilder.newSplitPanel(GridSize.COL50)
        gridBuilder.newFieldset(getString("fibu.projekt")).suppressLabelForWarning().let { fs ->
            // project
            projektSelectPanel =
        ...
        gridBuilder.newSplitPanel(GridSize.COL50)
        gridBuilder.newFieldset(getString("fibu.kunde")).suppressLabelForWarning().let { fs ->
            // customer
        ...
        gridBuilder.newFieldset(getString("fibu.projectManager")).let { fs ->
            // project manager
        ...
        gridBuilder.newSplitPanel(GridSize.COL33)
        gridBuilder.newFieldset(getString("fibu.headOfBusinessManager")).let { fs ->
        ...
        gridBuilder.newSplitPanel(GridSize.COL33)
        gridBuilder.newFieldset(getString("fibu.salesManager")).let { fs ->
            //sales manager
        ...
        gridBuilder.newFieldset(getString("fibu.auftrag.erfassung.datum")).let { fsEntryDate ->
            // erfassungsDatum
        ...
        gridBuilder.newSplitPanel(GridSize.SPAN2)
        gridBuilder.newFieldset(getString("fibu.auftrag.angebot.datum")).let { fsOrderDate ->
            // angebotsDatum
        ...
        gridBuilder.newSplitPanel(GridSize.SPAN2)
        gridBuilder.newFieldset(getString("fibu.auftrag.entscheidung.datum")).let { fsOrderDate ->
            // entscheidungsDatum
        ...
        gridBuilder.newSplitPanel(GridSize.COL50)
        gridBuilder.newFieldset(getString("fibu.auftrag.bindungsFrist")).let { fs ->
            // Bindungsfrist
        ...
        gridBuilder.newSplitPanel(GridSize.COL50)
        gridBuilder.newFieldset(getString("contactPerson")).let { fs ->
            // contact person
        ...
        gridBuilder.newFieldset(getString("fibu.auftrag.beauftragungsdatum")).let { fs ->
            // Beauftragungsdatum
        ...
        gridBuilder.newSplitPanel(GridSize.COL25)
        gridBuilder.newFieldset(getString("fibu.auftrag.forecastType")).let { fs ->
            // forecast type dropDown
            val forecastTypeRenderer = LabelValueChoiceRenderer<AuftragForecastType>(
                this,
                AuftragForecastType.values()
            )
            val forecastTypeActiveModel: IModel<AuftragForecastType> = PropertyModel(
                data,
                "forecastType"
            )
            val forecastTypeChoice = DropDownChoicePanel<AuftragForecastType>(
                fs.newChildId(), forecastTypeActiveModel, forecastTypeRenderer.values,
                forecastTypeRenderer, false
            )
            forecastTypeChoice.setNullValid(true)
            forecastTypeChoice.setTooltip(getString("fibu.auftrag.forecastType.info"))
            fs.add(forecastTypeChoice)
        }
        gridBuilder.newSplitPanel(GridSize.COL50)
        gridBuilder.newFieldset(getString("fibu.periodOfPerformance")).let { fs ->
        ...
        gridBuilder.newFieldset(getString("fibu.probabilityOfOccurrence")).let { fs ->
        ...
        gridBuilder.newSplitPanel(GridSize.COL50)
        object : ToggleContainerPanel(gridBuilder.panel.newChildId()) {
            /**
            ...
            paymentSchedulePanel!!.isVisible =
                data!!.paymentSchedules != null && data!!.paymentSchedules!!.isEmpty() == false
        ...
        gridBuilder.newSplitPanel(GridSize.COL50)
        gridBuilder.newFieldset(getString("comment")).let { fs ->
            // comment
            fs.add(MaxLengthTextArea(TextAreaPanel.WICKET_ID, PropertyModel(data, "bemerkung")), true)
        }
        gridBuilder.newSplitPanel(GridSize.COL50)
        gridBuilder.newFieldset(getString("fibu.auftrag.statusBeschreibung")).let { fs ->
            // status comment
            fs.add(
        ...
        gridBuilder.newFieldset(getString("attachments")).let { fs ->
            // attachments
            var attachments = ""
    ...
    private fun refreshPositions() {
        ...
        for (position in data!!.positionenIncludingDeleted!!) {
            ...
            run {
                ...
                if (WicketSupport.getAccessChecker()
                        .hasRight(user, RechnungDao.USER_RIGHT_ID, UserRightValue.READWRITE) == true
                ) {
            ...
            posGridBuilder.newFieldset(getString("fibu.periodOfPerformance")).let {fs->
                // Period of performance
            ...
            posGridBuilder.newSplitPanel(GridSize.COL50)
            posGridBuilder.newFieldset(getString("fibu.auftrag.forecastType")).let { fs ->
                // forecast type dropDown
                val forecastTypeRenderer = LabelValueChoiceRenderer<AuftragForecastType>(
                    this,
                    AuftragForecastType.values()
                ...
                forecastTypeChoice.setNullValid(true)
                forecastTypeChoice.setTooltip(getString("fibu.auftrag.forecastType.pos.info"))
                fs.add(forecastTypeChoice)
            }
    ...
    private fun positionInInvoiceExists(position: AuftragsPositionDO): Boolean {
        if (position.id != null) {
            val invoicePositionList =
                WicketSupport.get(RechnungCache::class.java).getRechnungsPosInfosByAuftragsPositionId(position.id)
