...
open class LantJdbcSettingsDialog(
  ...
  init {
    ...
    if (context.setupContext.dockerMode == SetupContext.DockerMode.STACK) {
      val hintLabel = Label("Please wait for the data-base of docker compose (try to retest connection).")
      hintLabel.foregroundColor = TextColor.ANSI.RED
      hintLabel.layoutData = GridLayout.createHorizontallyFilledLayoutData(2)
      contentPane.addComponent(Label("")).addComponent(hintLabel)
    }
    ...
    contentPane.addComponent(
      LanternaUtils.createButtonBar(context, unitWidth, false,
        jdbcTestButton,
