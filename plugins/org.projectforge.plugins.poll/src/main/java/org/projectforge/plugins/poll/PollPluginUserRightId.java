...
public enum PollPluginUserRightId implements IUserRightId
{
  PLUGIN_POLL("PLUGIN_POLL", "plugin30", "plugins.poll"),

  PLUGIN_POLL_ATTENDEE("PLUGIN_POLL_ATTENDEE", "plugin31", "plugins.poll"),

  PLUGIN_POLL_EVENT("PLUGIN_POLL_EVENT", "plugin32",
      "plugins.poll.event");
  public static class ProviderService implements RightRightIdProviderService
  {
    ...
    {
      return Arrays.asList(PollPluginUserRightId.values());
    }
  }
   ...
   * @param i18nKey
   */
  private PollPluginUserRightId(final String id, final String orderString, final String i18nKey)
  {
    this.id = id;
