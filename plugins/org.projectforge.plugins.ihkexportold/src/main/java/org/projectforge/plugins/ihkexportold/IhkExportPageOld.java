...
public class IhkExportPageOld extends AbstractStandardFormPage implements ISelectCallerPage
{
  private static final long serialVersionUID = -7157440416517271655L;

  private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(IhkExportPageOld.class);

  @SpringBean
  private transient TimesheetDao timesheetDao;

  private final IhkExportFormOld form;

  public IhkExportPageOld(final PageParameters parameters)
  {
    super(parameters);
    form = new IhkExportFormOld(this);
    body.add(form);
    form.init();
  }
  ...
  {
    return getString("plugins.ihkexportOld.title");
  }
  ...
  void export()
  {
    ...
    if (timeSheetList.size() <= 0) {
      form.addError("plugins.ihkexportOld.noitemsfound");
      return;
    }
    ...
    final String filename = "Wochenbericht_"
        ...
        + ".xls";
    byte[] xls = IhkExporterOld.getExcel(timeSheetList);
