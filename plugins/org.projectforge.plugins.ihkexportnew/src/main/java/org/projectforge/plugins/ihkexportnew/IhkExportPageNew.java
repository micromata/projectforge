...
public class IhkExportPageNew extends AbstractStandardFormPage implements ISelectCallerPage
{
  private static final long serialVersionUID = -7157440416517271655L;

  private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(IhkExportPageNew.class);

  @SpringBean
  private transient TimesheetDao timesheetDao;

  private final IhkExportFormNew form;

  public IhkExportPageNew(final PageParameters parameters)
  {
    super(parameters);
    form = new IhkExportFormNew(this);
    body.add(form);
    form.init();
  }
  ...
  {
    return getString("plugins.ihkexportNew.title");
  }
  ...
  void export()
  {
    ...
    if (timeSheetList.size() <= 0) {
      form.addError("plugins.ihkexportNew.noitemsfound");
      return;
    }
    ...
    final String filename = "Wochenbericht_"
        ...
        + ".xls";
    byte[] xls = IhkExporterNew.getExcel(timeSheetList);
