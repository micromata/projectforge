...
class MerlinPagesRest :
    ...
    override fun newBaseDO(request: HttpServletRequest?): MerlinTemplateDO {
        ...
        template.adminIds = "${ThreadLocalUserContext.loggedInUserId}"
    ...
    override fun transformFromDB(obj: MerlinTemplateDO, editMode: Boolean): MerlinTemplate {
        ...
        Group.restoreDisplayNames(dto.accessGroups)
        ...
        obj.id?.let { id ->
            val list =
                attachmentsService.getAttachments(jcrPath!!, id, attachmentsAccessChecker)
                    ...
                    UIRow()
                        .add(
                            UICol(md = 6)
                                .add(
                                    UILabel(
                                        "plugins.merlin.variables.input",
                                        tooltip = "plugins.merlin.variables.input.info"
                                    )
                                )
                                .add(
                                    variables
                            ...
                            UIRow()
                                .add(
                                    UICol()
                                        .add(
                                            UIReadOnlyField(
                                                "accessGroupsAsString",
                                                label = "plugins.merlin.accessGroups"
                                            )
                                        )
                                )
                        )
