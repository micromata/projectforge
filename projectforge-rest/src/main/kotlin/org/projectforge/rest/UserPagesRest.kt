...
class UserPagesRest
    ...
    override fun createListLayout(
        ...
        if (adminAccess) {
            agGrid.add(lc, PFUserDO::personalPhoneIdentifiers, PFUserDO::description)
        ...
        agGrid.add(UIAgGridColumnDef.createCol(lc, PFUserDO::gpgPublicKey, wrapText = true))
        ...
        if (accessChecker.isLoggedInUserMemberOfAdminGroup) {
            if (externalUserManagementSystem) {
                elements.add(
                    UIFilterListElement(
                        "sync",
                        label = translate("user.filter.syncStatus"),
                        defaultFilter = true,
                        multi = false
                    )
            ...
            elements.add(
                UIFilterListElement(
                    "status",
                    label = translate("user.filter.status"),
                    defaultFilter = true,
                    multi = false
                )
    ...
    private fun validatePosixValues(validationErrors: MutableList<ValidationError>, dto: User) {
        ...
        if (ldapValues.loginShell.isNullOrBlank()) {
            validationErrors.add(
                ValidationError.createFieldRequired(
                    "ldapValues.loginShell",
                    translate("ldap.loginShell")
                )
            )
        }
    }
    ...
    private fun addRights(layout: UILayout, dto: User) {
        ...
        for (rightDto in userRights) {
            ...
            if (right.isBooleanType) {
                ...
                uiElements.add(
                    UISelect(
                        "userRights[$counter].value",
                        label = right.id.i18nKey,
    ...
    companion object {
        internal fun createUserSettingsCol(uiLength: UILength, user: PFUserDO): UICol {
            ...
            val col = UICol(uiLength)
                ...
                .add(
                    UIReadOnlyField(
                        User::lastPasswordChangeFormatted.name,
                        label = "user.changePassword.password.lastChange"
                    )
                )
