...
class CalendarSubscriptionServiceRest {
    ...
    fun exportCalendar(request: HttpServletRequest): ResponseEntity<*> {
        ...
        try {
            ...
            val generator = ICalGenerator(exportVAlarms = "true" == params[PARAM_EXPORT_REMINDER])
            // read events
...
//    }
    ...
    private fun readEventsFromCalendars(generator: ICalGenerator, params: Map<String, String>): List<TeamCalDO>? {
        ...
        val vacationEvents =
            mutableSetOf<Long>() // For avoiding multiple entries of vacation days. Ids of vacation event.
    ...
    private fun readTimesheets(generator: ICalGenerator, timesheetUser: PFUserDO?) {
        ...
        for (timesheet in timesheetList) {
            ...
            if (startTime != null && stopTime != null) {
                val vEvent = generator.addEvent(startTime, stopTime, summary, uid)
                VEventUtils.addDescription(vEvent, timesheet.description)
                VEventUtils.addLocation(vEvent, timesheet.location)
            } else {
                log.error { "Oups, timesheet without start or stop time: $timesheet" }
            }
        }
    }
