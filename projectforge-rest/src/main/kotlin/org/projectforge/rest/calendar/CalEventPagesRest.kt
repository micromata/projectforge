...
class CalEventPagesRest() : AbstractDTOPagesRest<CalEventDO, CalEvent, CalEventDao>(
  ...
  override fun onBeforeGetItemAndLayout(request: HttpServletRequest, dto: CalEvent, userAccess: UILayout.UserAccess) {
    val startDate = PFDateTimeUtils.parseAndCreateDateTime(
      request.getParameter("startDate"),
      numberFormat = PFDateTime.NumberFormat.EPOCH_SECONDS
    )
    val endDate = PFDateTimeUtils.parseAndCreateDateTime(
      request.getParameter("endDate"),
      numberFormat = PFDateTime.NumberFormat.EPOCH_SECONDS
    )
    var origStartDate = PFDateTimeUtils.parseAndCreateDateTime(
      request.getParameter("origStartDate"),
      numberFormat = PFDateTime.NumberFormat.EPOCH_SECONDS
    )
    var origEndDate = PFDateTimeUtils.parseAndCreateDateTime(
      request.getParameter("origEndDate"),
      numberFormat = PFDateTime.NumberFormat.EPOCH_SECONDS
    )
    ...
    if (dto.id != null) {
      if (origStartDate != null && origEndDate != null && dto.hasRecurrence) {
        ...
        dto.selectedSeriesEvent = TeamEvent(
          startDate = origStartDate.sqlTimestamp,
      ...
      if (masterEvent?.startDate?.before(dto.selectedSeriesEvent?.startDate) != false) {
        radioButtonGroup.add(
          UIRadioButton(
            ...
            SeriesModificationMode.FUTURE,
            label = "plugins.teamcal.event.recurrence.change.future"
          )
        )
      } else {
        radioButtonGroup.add(
          UIRadioButton(
            "seriesModificationMode",
            SeriesModificationMode.ALL,
            label = "plugins.teamcal.event.recurrence.change.all"
          )
        )
      }
      radioButtonGroup.add(
        UIRadioButton(
          "seriesModificationMode",
          SeriesModificationMode.SINGLE,
          label = "plugins.teamcal.event.recurrence.change.single"
        )
      )
      layout.add(
        UIFieldset(12, title = "plugins.teamcal.event.recurrence.change.text")
