...
class SequenceConverter : PropertyConverter() {
    override fun toVEvent(event: CalEvent): Property? {
        return Sequence((if (event.sequence != null) event.sequence else 0)!!)
    }

    override fun fromVEvent(event: CalEvent, vEvent: VEvent): Boolean {
        val sequence = vEvent.sequence
        event.sequence = sequence?.sequenceNo ?: 0
