...
class VacationPagesRest :
    ...
    override fun createEditLayout(dto: Vacation, userAccess: UILayout.UserAccess): UILayout {
        ...
        val obj = VacationDO()
        dto.copyTo(obj)
        val availableStatusValues = vacationDao.getAllowedStatus(ThreadLocalUserContext.loggedInUser!!, obj)
                ...
                UIRow()
                    ...
                    .add(
                        UICol(3)
                            .add(
                                UIReadOnlyField(
                                    "vacationDaysLeftInYearString",
                                    lc,
                                    UIDataType.STRING,
                                    "vacation.availabledays"
                                )
                            )
                    )
            )
                ...
                UIRow()
                    ...
                    .add(
                        UICol(6)
                            .add(
                                UISelect("status", lc,
                                    values = availableStatusValues.map {
                                        UISelectValue(
                                            it.name,
                                            translate(it.i18nKey)
                                        )
                                    })
                            )
                    )
