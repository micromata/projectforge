    ...
    companion object {
        ...
        internal fun sync(
            ...
            if (contact.name != SipgateContactSyncDO.getName(address)) {
                if (syncInfo != null && syncInfo.fieldsInfo["name"] != SipgateContactSyncDO.SyncInfo.hash(contact.name)) {
                    ...
                    result.addOutdatedAddressField("name", contact.name, SipgateContactSyncDO.getName(address))
        ...
        internal fun sync(
            ...
            if (contactValue2Compare != addressValue2Compare) {
                ...
                contactField.setter.call(
                    contact,
                    addressValue
                ) // Only needed, if contact is outdated due to other fields.
            }
        }
        ...
        internal fun findMatches(syncContext: SyncContext)
            ...
            if (log.isDebugEnabled) {
                ...
                map.entries.forEach { entry ->
                    sb.appendLine(
                        "${entry.key}=[${
                            entry.value.sortedByDescending { it.score }.joinToString { "${it.addressId}:${it.score}" }
                        }]"
                    )
                }
        ...
        internal fun matchScore(contact: SipgateContact, address: AddressDO): Int {
            ...
            contact.emails?.forEach { email ->
                ...
                if (!str.isNullOrEmpty() && str == address.email?.trim()
                        ?.lowercase() || str == address.privateEmail?.trim()
                        ?.lowercase()
                ) {
                    ++counter
                }
            ...
            if (!address.organization.isNullOrBlank() && contact.organization?.trim()
                    ?.lowercase() == address.organization?.trim()
                    ?.lowercase()
            ) {
                ++counter
            }
                                ...
                                if (contact != null) {
                                    ...
                                    if (syncResult.contactOutdated || syncResult.addressDOOutdated) {
                                        log.info {
                                            ...
                                            }: Updating address and/or contact: $syncResult"
                                        }
                                    } else {
                                    ...
                                    if (resetContacts || syncResult.contactOutdated) {
                                        log.info {
                                            ...
                                            }: Updating remote contact: $contact, was: $oldContact"
                                        }
                                    ...
                                    log.info {
                                        ...
                                        }: Creating remote contact (doesn't yet exist): $address"
                                    }
                ...
                try {
                    if (configuration.updateSipgateContacts) {
                        ...
                        log.info {
                            ...
                            }: NOT deleting remote contact (see projectforge.properties): $contact"
                        }
                    }
