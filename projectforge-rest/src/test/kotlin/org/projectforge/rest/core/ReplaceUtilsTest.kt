...
class ReplaceUtilsTest {
    ...
    fun encodeFilenameTest() {
        ...
        assertEquals("_", ReplaceUtils.encodeFilename("  ", true))
        assertEquals("Kai_Oester_Test", ReplaceUtils.encodeFilename("Kai Öster:,Test", true))
        ...
        assertEquals("Ä_._._", ReplaceUtils.encodeFilename("Ä\"*/:.<>?.\\|", false))
        var sb = StringBuilder()
        for (ch in 0..31) {
            sb.append(ch.toChar())
        }
        sb.append("xxx").append(127.toChar())
        assertEquals("_xxx_", ReplaceUtils.encodeFilename(sb.toString(), false))
