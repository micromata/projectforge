...
/**
 * The invariant server-side paging rests on, for the outgoing invoice list: a paged result ([getListPage])
 * is the same result the non-paged `POST list` ([getList]) returns, only cut into pages — same rows, same
 * order, same total. If the two ever drift, the footer's total or a page boundary would lie. See
 * [OutgoingInvoiceEntityRest.sortIds] / [OutgoingInvoiceEntityRest.aggregate] and `MIGRATION-list-paging.md`.
 */
class OutgoingInvoiceListPageTest : AbstractTestBase() {
    @Autowired
    ...
    fun `paged result concatenates to the whole list sorted by the computed net sum`() {
        // Two equal sums (200) so the nummer tie-break is exercised: filterList and sortIds must resolve it
        // identically (both over the loaded invoices), or the two orderings would part at the tie.
    ...
    fun `a deleted invoice drops out of the paged list without a gap and shrinks the total`() {
        ...
        val removed = ids[2]
        rechnungDao.markAsDeleted(rechnungDao.find(removed, checkAccess = false)!!, checkAccess = false)
        ...
        // Still the same result the non-paged path returns, now over one fewer invoice.
    ...
    private fun collectPaged(filter: MagicFilter, pageSize: Int): PagedResult {
        ...
        while (true) {
            ...
            val page = getListPage(
                request, outgoingInvoiceEntityRest, rechnungDao, listPageCache, filter.clone(), offset, pageSize, false,
            )
    ...
    private fun wholeListIds(filter: MagicFilter): List<Long> =
        getList(request, outgoingInvoiceEntityRest, rechnungDao, filter.clone()).resultSet.mapNotNull { it.id }
    ...
    private fun insertInvoices(vararg netSums: Int): List<Long> {
        return netSums.map { net ->
            ...
            // Sequential number, assigned right before the insert that bumps the max (validate enforces it).
