...
class TimesheetMassUpdateTest : AbstractTestBase() {
  ...
  fun massUpdateWithKost2() {
    ...
    Assertions.assertEquals(getTask(prefix + "1.1").id, dbList.find { it.description == "TS#0" }?.taskId) // Not moved.
    Assertions.assertEquals(getTask(prefix + "1.2").id, dbList.find { it.description == "TS#1" }?.taskId) // Not moved.
    Assertions.assertEquals(getTask(prefix + "1.2").id, dbList.find { it.description == "TS#2" }?.taskId) // Not moved.
  ...
  private fun massUpdate(list: List<TimesheetDO>, master: TimesheetDO): List<TimesheetDO> {
