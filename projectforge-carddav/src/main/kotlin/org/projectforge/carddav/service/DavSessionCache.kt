...
/**
 * Work around for authenticate carddav clients. Expires after 1 minute.
 */
@Service
open class DavSessionCache
    ...
    override fun getSessionId(request: HttpServletRequest): String? {
        request.requestURI.let { uri ->
            val usersPath = if (CardDavInit.cardDavBasePath == "/") {
                "/users/"
            } else {
                "${CardDavInit.cardDavBasePath}/users/"
            }
            if (!CardDavFilter.urlMatches(request)) {
                log.info { "uri doesn't start with $usersPath: $uri" }
                return null
            }
            val username = uri.removePrefix(usersPath).substringBefore('/')
            if (username.isBlank()) {
                log.info { "uri doesn't contain username after $usersPath: $uri" }
                return null
            }
