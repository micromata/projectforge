...
ProjectForge searches automatically for `$HOME/ProjectForge/projectforge.properties`. You may specify another location:
Start with Your own config: `java -jar target/projectforge-application-X.X.X.jar --spring.config.additional-location=file:**mypath**/projectforge.properties`
