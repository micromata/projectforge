...
3. Increase version number of all `pom.xml` files: +
   `export PF_NEW_VERSION=6.26.0` +
   `find . -name \\*.xml | xargs perl -pi -e 's|<version>[0-9\.\-A-Z]*</version><!-- projectforge.version -\->|<version>$ENV{PF_NEW_VERSION}</version><!-- projectforge.version -\->|g'`
