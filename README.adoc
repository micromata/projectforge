...
ProjectForge searches automatically for `$HOME/ProjectForge/projectforge.properties`.
...
3. Change version number of all `pom.xml` files: +
   ...
   `find . -name pom.xml | xargs perl -pi -e 's|<version>[0-9\.\-A-Z]*</version><!-- projectforge.version -\->|<version>$ENV{PF_NEW_VERSION}</version><!-- projectforge.version -\->|g'`
