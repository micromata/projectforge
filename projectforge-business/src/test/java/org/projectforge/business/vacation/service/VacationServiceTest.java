...
public class VacationServiceTest {
  ...
  public void testUpdateUsedVacationDaysFromLastYearLaterThanEndDate() {
    ...
    vacationData.setEmployee(employee);
    vacationData.setStartDate(now.with(Month.APRIL).withDayOfMonth(1));
    vacationData.setEndDate(now.withMonth(4).withDayOfMonth(10));
  ...
  public void testUpdateUsedVacationDaysFromLastYearZero() {
    ...
    vacationData.setSpecial(false);
    vacationData.setEmployee(employee);
    vacationData.setStartDate(now.with(Month.MARCH).withDayOfMonth(1));
    vacationData.setEndDate(now.with(Month.MARCH).withDayOfMonth(10));
    BigDecimal newValue = this.vacationService.updateUsedVacationDaysFromLastYear(vacationData);
    assertEquals(newValue, BigDecimal.ZERO);
  }
  ...
  public void testUpdateUsedVacationDaysFromLastYearFillUpRest() {
    endLastYear = endLastYear.withYear(now.getYear()).with(Month.MARCH).withDayOfMonth(31);
    when(configService.getEndDateVacationFromLastYear()).thenReturn(endLastYear);
  ...
  public void testUpdateUsedVacationDaysFromLastYearOverEndDate() {
    ...
    when(employee.getAttribute(VacationAttrProperty.PREVIOUSYEARLEAVEUSED.getPropertyName(), BigDecimal.class)).thenReturn(new BigDecimal(1));
    vacationData.setEmployee(employee);
    vacationData.setStartDate(now.with(Month.MARCH).withDayOfMonth(25));
    vacationData.setEndDate(now.with(Month.APRIL).withDayOfMonth(5));
    BigDecimal numberOfDays = vacationService.getVacationDays(vacationData.getStartDate(), endLastYear, vacationData.getHalfDay());
    BigDecimal newValue = this.vacationService.updateUsedVacationDaysFromLastYear(vacationData);
    assertEquals(newValue, BigDecimal.ONE.add(numberOfDays));
  }
  ...
  public void testUpdateUsedVacationDaysFromLastYearFirstNormalAfterIsSpecial() {
    ...
    final LocalDate startDate = now.with(Month.MARCH).withDayOfMonth(1);
    vacationData.setStartDate(startDate);
    vacationDataOld.setStartDate(startDate);
    final LocalDate endDate = now.with(Month.MARCH).withDayOfMonth(10);
    vacationData.setEndDate(endDate);
    vacationDataOld.setEndDate(endDate);
  ...
  public void testGetAvailableVacationdaysNoDaysUsed() {
    ...
    when(vacationDao.getActiveVacationForYear(employee, now.getYear(), false)).thenReturn(vacationList);
    when(employee.getUrlaubstage()).thenReturn(30);
    BigDecimal availableVacationdays = vacationService.getAvailableVacationdaysForYear(employee, now.getYear(), false);
  ...
  public void testGetAvailableVacationdaysDaysUsed() {
    ...
    vacation.setStartDate(LocalDate.now().with(Month.MARCH).withDayOfMonth(25));
    vacation.setEndDate(LocalDate.now().with(Month.APRIL).withDayOfMonth(5));
  ...
  public void testUpdateUsedNewVacationDaysFromLastYearUsedPartial1() throws ParseException {
    ...
    when(employee.getUrlaubstage()).thenReturn(30);
    when(vacationDao.getVacationForPeriod(same(employee), any(LocalDate.class), any(LocalDate.class), eq(false))).thenReturn(vacationList);
