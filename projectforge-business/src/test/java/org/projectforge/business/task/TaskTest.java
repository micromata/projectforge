...
public class TaskTest extends AbstractTestNGBase
{
  ...
  public void checkAccess()
  {
    ...
    initTestDB.addGroup("checkAccessTestGroup", new String[] { AbstractBase.TEST_USER });
    ...
    checkAccess(AbstractBase.TEST_ADMIN_USER, task.getId(), projekt, kost2);
    checkAccess(AbstractBase.TEST_USER, task.getId(), projekt, kost2);
  }
  ...
  public void checkKost2AndTimesheetBookingStatusAccess()
  {
    ...
    final GroupDO projectManagers = initTestDB.addGroup(groupName,
        new String[] { AbstractBase.TEST_PROJECT_MANAGER_USER, AbstractBase.TEST_PROJECT_ASSISTANT_USER,
                AbstractBase.TEST_USER });
  ...
  public void readTotalDuration()
  {
    ...
    TimesheetDO ts = new TimesheetDO().setUser(getUser(AbstractBase.TEST_USER)).setStartDate(dh.getDate())
    ...
    ts = new TimesheetDO().setUser(getUser(AbstractBase.TEST_USER)).setStartDate(dh.add(Calendar.HOUR_OF_DAY, 1).getDate())
        .setStopTime(dh.add(Calendar.HOUR_OF_DAY, 4).getTimestamp()).setTask(task);
    timesheetDao.save(ts);
