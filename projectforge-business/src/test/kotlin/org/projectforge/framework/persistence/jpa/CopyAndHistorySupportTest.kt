...
class CopyAndHistorySupportTest : AbstractTestBase() {
    @Test
    fun baseTests() {
        val src = ContractDO()
        val dest = ContractDO()
    ...
    ): CopyAndHistorySupport.Context {
        var context = CopyAndHistorySupport.Context(debug = debug, createHistory = createHistory)
        CopyAndHistorySupport.copyValues(src, dest, context)
        Assertions.assertEquals(expectedStatus, context.currentCopyStatus)
        context = CopyAndHistorySupport.Context(debug = debug, createHistory = createHistory)
        CopyAndHistorySupport.copyValues(src, dest, context)
