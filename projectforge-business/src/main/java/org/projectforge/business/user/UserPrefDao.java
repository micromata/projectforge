...
public class UserPrefDao extends BaseDao<UserPrefDO> {
    ...
    public String[] getPrefNames(final String area) {
        ...
        List<String> names = persistenceService.namedQuery(
                UserPrefDO.FIND_NAMES_BY_USER_AND_AREA,
                String.class,
                new Pair<>("userId", user.getId()),
                new Pair<>("area", area));
    ...
    public List<UserPrefDO> getListWithoutEntries(String areaId) {
        final PFUserDO user = ThreadLocalUserContext.getUser();
        final List<Object[]> list = persistenceService.namedQuery(
                UserPrefDO.FIND_IDS_AND_NAMES_BY_USER_AND_AREA,
                Object[].class,
                new Pair<>("userId", user.getId()),
                new Pair<>("area", areaId));
    ...
    public boolean doesParameterNameAlreadyExist(final Integer id, final Integer userId, final String areaId,
        ...
        if (id != null) {
            userPref = persistenceService.selectNamedSingleResult(
        ...
        for (final Field field : fields) {
            if (field.isAnnotationPresent(UserPrefParameter.class)) {
                ...
                if (obj != null) {
                    ...
                    userPrefEntry.setValueAsObject(value);
                }
                evaluateAnnotation(userPrefEntry, beanType, field);
                if (userPrefEntry.getOrderString() == null) {
                    userPrefEntry.setOrderString("ZZZ" + StringHelper.format2DigitNumber(no++));
                }
    ...
    private void evaluateAnnotations(final UserPrefDO userPref, final Class<?> beanType) {
        ...
        for (final Field field : fields) {
            if (field.isAnnotationPresent(UserPrefParameter.class)) {
                ...
                if (StringUtils.isBlank(userPrefEntry.getOrderString())) {
                    userPrefEntry.setOrderString("ZZZ" + StringHelper.format2DigitNumber(no++));
                }
                userPrefEntry.setParameter(field.getName());
            }
    ...
    private void evaluateAnnotation(final UserPrefEntryDO userPrefEntry, final Class<?> beanType, final Field field) {
        ...
        userPrefEntry.setI18nKey(ann.i18nKey());
        userPrefEntry.setTooltipI18nKey(ann.tooltipI18nKey());
        userPrefEntry.setDependsOn(StringUtils.isNotBlank(ann.dependsOn()) ? ann.dependsOn() : null);
        userPrefEntry.setRequired(ann.required());
        userPrefEntry.setMultiline(ann.multiline());
        userPrefEntry.setOrderString(StringUtils.isNotBlank(ann.orderString()) ? ann.orderString() : null);
        if (String.class.isAssignableFrom(field.getType())) {
            userPrefEntry.setMaxLength(HibernateMetaModel.getPropertyLength(beanType, field.getName()));
        }
        userPrefEntry.setType(field.getType());
    }
    ...
    public void updateParameterValueObject(final UserPrefEntryDO userPrefEntry) {
        userPrefEntry.setValueAsObject(getParameterValue(userPrefEntry.getType(), userPrefEntry.getValue()));
    }
