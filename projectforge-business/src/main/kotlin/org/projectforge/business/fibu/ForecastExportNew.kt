...
open class ForecastExportNew { // open needed by Wicket.
    ...
    enum class ForecastCols(val col: Int) {
        ORDER_NR(1), POS_NR(2), DATE_OF_OFFER(3), DATE(4), DATE_OF_DECISION(5), HEAD(6), PROJECT(7)
        //Titel	Titel	Art	Abrechnungsart	Auftrag Status	Position Status	PT	Nettosumme	fakturiert	Noch zu fakturieren	vollständig fakturiert	Debitorenrechnungen	Leistungszeitraum		Eintrittswahrsch. in %	Ansprechpartner	Strukturelement	Bemerkung	Wahrscheinlichkeitswert	Monatsende Startdatum +1	Monatsende Enddatum +1	Anzahl Monate
    }
    ...
    private class Context(val workbook: ExcelWorkbook, val forecastSheet: ExcelSheet) {
        var doubleCellStyle = workbook.createOrGetCellStyle("DataFormat.float")

        init {
            doubleCellStyle.setDataFormat(workbook.getDataFormat("#.#"))
        }
    }
    ...
    private fun setDoubleValue(ctx: Context, sheet: ExcelSheet, row: Int, col: Int, value: Double?) {
        val cell = sheet.getCell(row, col)
        if (value == null) cell.setBlank()
        else {
            cell.setCellValue(value)
            cell.cellStyle = ctx.doubleCellStyle
        }
    }

    private fun setStringValue(ctx: Context, sheet: ExcelSheet, row: Int, col: Int, value: String?) {
        val cell = sheet.getCell(row, col)
        if (value == null) cell.setBlank()
    ...
    private fun addOrderPosition(ctx: Context, row: Int, order: AuftragDO, pos: AuftragsPositionDO, startDate: PFDate) {
        ...
        setDoubleValue(ctx, sheet, row, ForecastCols.ORDER_NR.col, order.nummer?.toDouble())
        setStringValue(ctx, sheet, row, ForecastCols.POS_NR.col, "#${pos.number}")
