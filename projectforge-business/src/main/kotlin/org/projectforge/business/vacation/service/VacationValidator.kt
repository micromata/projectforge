...
object VacationValidator {
    ...
    internal fun validate(
        ...
        if (vacation.special == true) {
            ...
            var allVacationEntries =
                vacationService.getVacationsListForPeriod(employee, yearPeriod.begin, yearPeriod.end, false)
            if (dbVacation != null) {
            ...
            if (numberOfWorkingDays > stats.vacationDaysLeftInYearWithoutCarry) {
                ...
                if (year != endDate.year) { // Leave days over at least 2 years...
                    ...
                    val numberOfWorkingDaysInStartDate =
                        VacationService.getVacationDays(
                            vacation,
                            startDay.beginOfYear.localDate,
                            startDay.endOfYear.localDate
                        )
                    ...
                    numberOfWorkingDays =
                        VacationService.getVacationDays(
                            vacation,
                            endDay.beginOfYear.localDate,
                            endDay.endOfYear.localDate
                        )
                    stats =
                        vacationService.getVacationStats(employee, endDate.year, vacationEntries = allVacationEntries)
                }
                ...
                if (modifiedStartDate.isBefore(endOfVacationYear)) {
                    val overlapDays = if (endDate > endOfVacationYear) {
                        // Part of the vacation is in the overlap period.
                        VacationService.getVacationDays(
                            modifiedStartDate,
                            endOfVacationYear,
                        ...
                        // The whole vacation is in the overlap period.
                        BigDecimal.ZERO
                    }
                    // Additional carry days from previous year:
                    // The remaining leave from previous year can be used in the overlap period.
                    val additionalCarryDays =
                        maxOf(stats.remainingLeaveFromPreviousYearUnused!! - overlapDays, BigDecimal.ZERO)
