...
open class TeamEventDO : DefaultBaseDO(), ICalendarEvent, Cloneable {
    ...
    @Transient
    private var recurrenceRuleObject: RRule<Temporal>? = null
    ...
    /**
     * If not given the recurrence will never end. Identifies the last possible event occurrence.
     *
     ...
     *
     * This field exist only for database query purposes.
     */
    ...
    open var reminderDuration: Long? = null
     ...
     */
    val recurrenceObject: Recur<Temporal>?
        @Transient
        get() {
    ...
    fun setRecurrence(rRule: RRule<ZonedDateTime>?): TeamEventDO {
        ...
        this.recurrenceUntil = PFDateTime.fromOrNull(recur.until)?.utilDate
    ...
    fun setRecurrence(recurrenceData: TeamEventRecurrenceData?): TeamEventDO {
        ...
        val recur = Recur.Builder<Temporal>()
            ...
            .frequency(ICal4JUtils.getCal4JFrequency(recurrenceData.frequency))
            .build()
        ...
        if (recurrenceData.frequency == RecurrenceFrequency.WEEKLY) {
            ...
            for (i in 0..11) {
                if (months[i]) {
                    // Month is one-based (1-Januar, 12-December):
                    recur.monthList.add(Month.valueOf(i + 1))
                }
            }
        ...
        val rrule = RRule<Temporal>(recur)
     ...
     */
    @Transient
    fun getRecurrenceRuleObject(): RRule<Temporal>? {
