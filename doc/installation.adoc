...
==== Built-in database
1. First start `docker run -t -i -p 127.0.0.1:8080:8080 -v $HOME/ProjectForge:/ProjectForge --name projectforge-app kreinhard/projectforge:latest`
...
2. First start: `docker-compose -p 127.0.0.1:8080:8080 run projectforge-app`
