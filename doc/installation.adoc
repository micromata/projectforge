...
purged after 30 days, but the monthly backups at the first of every month will be kept.

=== DB backup`
You may configure a purge job in `projectforge.properties`:
```# If purgeBackupDir is given and exists, ProjectForge will purge daily backups older than 30 days, but monthly backups will be kept.
