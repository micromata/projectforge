...
To use NGINX as a reverse proxy, it's necessary to create a configuration file. The standard path for NGINX configurations is `/etc/nginx/sites-available/`, so let's create the file xref:misc/nginx_sites-available_projectforge[`/etc/nginx/sites-available/projectforge`].
...
* Example start script: xref:misc/startProjectForge.sh[startProjectForge.sh]
* Example stop script: xref:misc/stopProjectForge.sh[stopProjectForge.sh]
